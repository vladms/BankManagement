...
public class RetreatMoneyScreen {
	...
	public RetreatMoneyScreen(AppInterfaceButtonEvents buttonEventsHandler) {
	...
	private void prepareGUI() {
		mainFrame = new JFrame("Retreat money");
	...
	private void prepareClientSelection() {
		...
		deleteButton = new JButton("Retreat money");
		deleteButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				} else if (!(sumTextField.getText().length() > 0) || (Double.parseDouble(sumTextField.getText())) < 0) {
					JOptionPane.showMessageDialog(mainFrame, "Sum is not in the good format!");
				} else{
					mainFrame.setVisible(false);
					mainFrame.remove(controlPanel);
					buttonEventHandler.retreatMoney(Integer.parseInt(accountIdTextField.getText()), Integer.parseInt(clientIdTextField.getText()), Double.parseDouble(sumTextField.getText()));
